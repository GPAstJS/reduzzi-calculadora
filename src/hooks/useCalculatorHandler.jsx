...
export const useCalculatorHandler = () => {
    const handleCalculatorData = async (data, openModal) => {
        let {
            // celular,
            // destinacaoObra,
            // faseObra,
        ...
        switch (true) {
            ...
            case m2Construcao >= 300 && m2Construcao <= 400:
                tipoDeConstrucaoPorcentagem =
                    constants.deTrezentosAQuatrocentos;
                break;
            default:
        ...
        const limitFinalData = new Date();
        limitFinalData.setMonth(9);
        limitFinalData.setFullYear(2021);

        if (dataFinal.getTime() <= limitFinalData.getTime()) {
            openModal();
        ...
        terminoMenorOuIgualQueAtual =
            dataFinal.getTime() <= new Date().getTime();
        ...
        valorMesRetroativo =
            metragemPorMes * constants.descontoPorMesRetroativo;
        ...
        switch (true) {
            case metroTotal <= 100:
                honorarioValor =
                    Math.round(
                        (totalImpostoSemReducao - regraAtual) / 2 / 100,
                    ) * 100;
                break;
            case metroTotal <= 200:
        ...
        if (terminoMenorOuIgualQueAtual) {
            ...
            valorMesRetroativo =
                mesesALancarNormal >= 0 ? mesesALancarNormal : 0;
            ...
            valorFinalDaObra =
                regraAtual -
                valorMesRetroativo -
                mesesALancar * Number(import.meta.env.VITE_DESCONTO_METRAGEM);
        }
        ...
        if (!terminoMenorOuIgualQueAtual) {
            if ((valorFinalDaObra + valorFinalDaObra * 0.2) / 60 < 100) {
                ...
                if (valorDoParcelamentoTotal < 100)
                    valorDoParcelamentoTotal = 100;
            }
        ...
        localStorage.setItem('obraData', JSON.stringify(payload));
